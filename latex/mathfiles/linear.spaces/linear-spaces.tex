...
\begin{example}[Distance of two vectors in $\mathbb{R}^{n}$]
    ...
    \begin{equation}
        \begin{split}
            |w|^{2} = \sum_{i=1}^{n}{\left(u_i^{2}\right)} + \sum_{i=1}^{n}{\left(v_i^{2}\right)} - 2 \sum_{i=1}^{n}{u_i v_i}
        \end{split}
    \end{equation}
    ...
    \begin{equation}
        \begin{split}
            |w|^{2} = v \cdot v + u \cdot u - 2u \cdot v
        \end{split}
    \end{equation}
    ...
    between $u$ and $u$ is zero, $\cos\theta = 0$.
\end{example}
...
present when we measure elements in Euclidean spaces, as segments or vectors.

The dot product is not ``derived'' in a way most things are. Instead, it is
useful because it simply ``appears'' in measurement.

\textbf{The inner product} is a generalization of the dot product in more general spaces.
Each space can have a different definition for its inner product. For example 

\begin{example}[Inner product of a functional space $C(a,b)$]

    Let $f:\mathbb{R} \to \mathbb{R}$ and $g:\mathbb{R} \to \mathbb{R}$ be continuous functions 
    in an interval $[a,b]$, the inner product is defined as
    \begin{equation}
        \begin{split}
