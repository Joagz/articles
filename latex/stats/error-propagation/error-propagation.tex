...
\begin{equation}
    \begin{split}
        ...
        &=E\left[\sum_{k}{a_{ik}(X_k-\mu_x^{(k)})}\sum_{l}{a_{lj}(X_l-\mu_x^{(l)})}\right] \\
        &=E\left[\sum_{k}{a_{ik}^{2}(X_k-\mu_x^{(k)})^{2}} + \mathop{\sum\sum}_{l\neq k}{a_{lj}a_{ik}(X_l-\mu_x^{(l)})(X_k-\mu_x^{(k)})}\right] \\
        &=E\left[\sum_{k}{a_{ik}^{2}(X_k-\mu_x^{(k)})^{2}}\right]+E\left[\mathop{\sum\sum}_{l\neq k}{a_{lj}a_{ik}(X_l-\mu_x^{(l)})(X_k-\mu_x^{(k)})}\right] \\
        &=\sum_{k}a_{ik}^{2}E\left[{(X_k-\mu_x^{(k)})^{2}}\right]+\mathop{\sum\sum}_{l\neq k}a_{lj}a_{ik}E\left[{(X_l-\mu_x^{(l)})(X_k-\mu_x^{(k)})}\right] \\
        &=\sum_{k}a_{ik}^{2}\Sigma_x^{kk}+\mathop{\sum\sum}_{l\neq k}a_{lj}a_{ik}\Sigma_x^{(lk)} \\
        &=\mathop{\sum\sum}_{k,l}{a_{lj}a_{ik}\Sigma_x^{(lk)}} \\
        &=\mathop{\sum\sum}_{k,l}{\dfrac{\partial f_l}{\partial X_j} \dfrac{\partial f_i}{\partial X_k}\Sigma_x^{(lk)}}
    \end{split}
\end{equation}
...
\begin{equation}
    \begin{split}
        \mathop{\sum\sum}_{k,l}{\dfrac{\partial f_l}{\partial X_j} \dfrac{\partial f_i}{\partial X_k}\Sigma_x^{(lk)}} = (J_{x})_{:,j} \cdot \Sigma_x \cdot (J_{x})_{i,:}
    \end{split}
\end{equation}
Aquí, utilizamos la notación ``$:,j$'' para indicar que extraemos la columna $j$ de la matriz, y ``$i,:$'' la fila $i$. Finalmente, si cada entrada de $\Sigma_y$ corresponde 
a (23) en la posición $ij$, tenemos que la matriz de covarianza de $Y$ es 
\begin{equation}
